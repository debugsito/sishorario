  ...
  $('.agregarambiente').click(function(){
    ...
    var fechai = $('.FechaInicio').val();
      ...
      if (fechaf =="") {
        ...
        var obtener = $('.form_ambientes').serialize();
		alert(obtener);
        $.ajax({
            type: "POST",
            url: "funciones/RegistrarAmbiente.php",
            data: obtener,
            success: function() {
                alertify.success('Ficha Registrada Exitosamente!'); //Mensaje de Datos Correctamente Insertados
                $('.tabla_ambientes').load("funciones/mostrarTablaAmbientes.php"); //Recargamos la Tabla(Para que se muestren los Nuevos Resultados)
